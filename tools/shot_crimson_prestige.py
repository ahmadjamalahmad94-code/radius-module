# -*- coding: utf-8 -*-
"""رندر + لقطة لقالب «القرمزي الفاخر» (crimson_prestige) على جوّال 390px وسطح 1280px."""
import os, re, sys
sys.stdout.reconfigure(encoding="utf-8", errors="replace")
REPO = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, REPO)
OUT = os.path.join(REPO, "preview", "crimson_prestige"); os.makedirs(OUT, exist_ok=True)
...
with app.app_context():
    ...
    vals = {"TENANT_NAME": "Hoberadius WiFi", "ACCENT_COLOR": "#DC2626",
            "BG_COLOR": "#120608", "WELCOME_TEXT": "تجربة بريميوم فاخرة بلا حدود",
            "SUPPORT_PHONE": "0590000000", "MOTIF_ICON": "coffee"}
    html = ht.render("crimson_prestige", vals, tenant_id=1)
...
html = re.sub(r'\$\([^)]*\)', '', html)
path = os.path.join(OUT, "crimson_prestige.html")
...
with sync_playwright() as p:
    ...
    pg.screenshot(path=os.path.join(OUT, "crimson_prestige_390.png"), full_page=True)
    pg.screenshot(path="C:/Projects/_review_crimson_prestige_390.png", full_page=True)
    info = pg.evaluate(r"""()=>{
    ...
    pg2.screenshot(path=os.path.join(OUT, "crimson_prestige_desktop.png"), full_page=True)
    pg2.screenshot(path="C:/Projects/_review_crimson_prestige_desktop.png", full_page=True)
    b.close()
print("rendered:", path)
