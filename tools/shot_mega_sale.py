# -*- coding: utf-8 -*-
"""رندر + لقطة لقالب «التخفيضات» (mega_sale) على جوّال 390px وسطح 1280px."""
import os, re, sys
sys.stdout.reconfigure(encoding="utf-8", errors="replace")
REPO = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, REPO)
OUT = os.path.join(REPO, "preview", "mega_sale"); os.makedirs(OUT, exist_ok=True)
...
with app.app_context():
    ...
    vals = {"TENANT_NAME": "Hoberadius WiFi", "ACCENT_COLOR": "#E11D48",
            "BG_COLOR": "#FFE7EE", "WELCOME_TEXT": "تخفيضات كبرى لفترة محدودة",
            "SUPPORT_PHONE": "0590000000", "MOTIF_ICON": "coffee"}
    html = ht.render("mega_sale", vals, tenant_id=1)
...
html = re.sub(r'\$\([^)]*\)', '', html)
path = os.path.join(OUT, "mega_sale.html")
...
with sync_playwright() as p:
    ...
    pg.screenshot(path=os.path.join(OUT, "mega_sale_390.png"), full_page=True)
    pg.screenshot(path="C:/Projects/_review_mega_sale_390.png", full_page=True)
    info = pg.evaluate(r"""()=>{
    ...
    pg2.screenshot(path=os.path.join(OUT, "mega_sale_desktop.png"), full_page=True)
    pg2.screenshot(path="C:/Projects/_review_mega_sale_desktop.png", full_page=True)
    b.close()
print("rendered:", path)
