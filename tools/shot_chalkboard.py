# -*- coding: utf-8 -*-
"""رندر + لقطة لقالب «اللوح الطباشيري» (chalkboard) على جوّال 390px وسطح 1280px."""
import os, re, sys
sys.stdout.reconfigure(encoding="utf-8", errors="replace")
REPO = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, REPO)
OUT = os.path.join(REPO, "preview", "chalkboard"); os.makedirs(OUT, exist_ok=True)
...
with app.app_context():
    ...
    html = ht.render("chalkboard", vals, tenant_id=1)
...
html = re.sub(r'\$\([^)]*\)', '', html)
path = os.path.join(OUT, "chalkboard.html")
...
with sync_playwright() as p:
    ...
    pg.screenshot(path=os.path.join(OUT, "chalkboard_390.png"), full_page=True)
    pg.screenshot(path="C:/Projects/_review_chalkboard_390.png", full_page=True)
    info = pg.evaluate(r"""()=>{
    ...
    pg2.screenshot(path=os.path.join(OUT, "chalkboard_desktop.png"), full_page=True)
    pg2.screenshot(path="C:/Projects/_review_chalkboard_desktop.png", full_page=True)
    b.close()
print("rendered:", path)
