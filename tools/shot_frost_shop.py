# -*- coding: utf-8 -*-
"""رندر + لقطة لقالب «الزجاج الثلجي» (frost_shop) على جوّال 390px وسطح 1280px."""
import os, re, sys
sys.stdout.reconfigure(encoding="utf-8", errors="replace")
REPO = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, REPO)
OUT = os.path.join(REPO, "preview", "frost_shop"); os.makedirs(OUT, exist_ok=True)
...
with app.app_context():
    ...
    vals = {"TENANT_NAME": "Hoberadius WiFi", "ACCENT_COLOR": "#1D4ED8",
            "BG_COLOR": "#E6F1FC", "WELCOME_TEXT": "تشكيلة الموسم بإطلالة باردة أنيقة",
            "SUPPORT_PHONE": "0590000000", "MOTIF_ICON": "coffee"}
    html = ht.render("frost_shop", vals, tenant_id=1)
...
html = re.sub(r'\$\([^)]*\)', '', html)
path = os.path.join(OUT, "frost_shop.html")
...
with sync_playwright() as p:
    ...
    pg.screenshot(path=os.path.join(OUT, "frost_shop_390.png"), full_page=True)
    pg.screenshot(path="C:/Projects/_review_frost_shop_390.png", full_page=True)
    info = pg.evaluate(r"""()=>{
    ...
    pg2.screenshot(path=os.path.join(OUT, "frost_shop_desktop.png"), full_page=True)
    pg2.screenshot(path="C:/Projects/_review_frost_shop_desktop.png", full_page=True)
    b.close()
print("rendered:", path)
