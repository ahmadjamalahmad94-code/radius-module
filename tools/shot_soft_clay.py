# -*- coding: utf-8 -*-
"""رندر + لقطة لقالب «الكلاي الناعم» (soft_clay) على جوّال 390px وسطح 1280px."""
import os, re, sys
sys.stdout.reconfigure(encoding="utf-8", errors="replace")
REPO = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, REPO)
OUT = os.path.join(REPO, "preview", "soft_clay"); os.makedirs(OUT, exist_ok=True)
...
with app.app_context():
    ...
    vals = {"TENANT_NAME": "Hoberadius WiFi", "ACCENT_COLOR": "#E8927C",
            "BG_COLOR": "#FCE7E2", "WELCOME_TEXT": "قهوة لطيفة وإنترنت سريع — يومك أحلى",
            "SUPPORT_PHONE": "0590000000", "MOTIF_ICON": "coffee"}
    html = ht.render("soft_clay", vals, tenant_id=1)
...
html = re.sub(r'\$\([^)]*\)', '', html)
path = os.path.join(OUT, "soft_clay.html")
...
with sync_playwright() as p:
    ...
    pg.screenshot(path=os.path.join(OUT, "soft_clay_390.png"), full_page=True)
    pg.screenshot(path="C:/Projects/_review_soft_clay_390.png", full_page=True)
    info = pg.evaluate(r"""()=>{
    ...
    pg2.screenshot(path=os.path.join(OUT, "soft_clay_desktop.png"), full_page=True)
    pg2.screenshot(path="C:/Projects/_review_soft_clay_desktop.png", full_page=True)
    b.close()
print("rendered:", path)
