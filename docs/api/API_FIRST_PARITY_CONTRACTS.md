...
| Method | Path | Mirrors | Notes |
|---|---|---|---|
| GET | `/api/v1/events-center` | `events_center` | Filters: `category, severity, actor_type, actor_id, target_type, target_id, correlation_id, from, to`. → `{events, count, summary}`. |
| GET | `/api/v1/events-center/{event_id}` | `events_detail` | → `{event, timeline}`. 404 if missing. |
| GET | `/api/v1/events-center/risk` | `events_risk` GET | → `{flags, summary}`. |
| POST | `/api/v1/events-center/risk/run` | `events_risk` POST | Runs risk rules. → `{result, flags}`. |
| GET | `/api/v1/events-center/security` | `events_security` | → `{events (category=security), flags (open)}`. |
| GET | `/api/v1/events-center/investigations` | `events_investigations` GET | `?status=`. → `{investigations}`. |
| POST | `/api/v1/events-center/investigations` | `events_investigations` POST | Body `{title*, severity?, entity_type?, entity_id?, summary?}`. → `201 {investigation}`. 422 missing title. |
