"""Render the subscriber-notifications page (default rules) with /static
rewritten to file:// for an offline headless screenshot of the per-template SMS
counters.
"""
...
os.environ["FLASK_SECRET"] = "render-subnotif-secret"
...
db_file = os.path.join(tempfile.mkdtemp(), "render_subnotif.db")
...
with flask_app.app_context():
    ...
    # Connect SMS so the channel shows as ready in the editor.
    tenant_sms_settings_repo.upsert(tenant_id=1, api_key="tw_x", sender="HOBE", enabled=True)
...
with flask_app.test_client() as client:
    ...
    res = client.get("/admin/radius/subscriber-notifications", follow_redirects=False)
    ...
    out = os.path.join(OUT_DIR, "sub_notif.html")
