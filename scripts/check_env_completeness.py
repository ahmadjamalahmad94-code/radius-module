...
def _extract_env_keys_from_ast(path: Path) -> set[str]:
    """يستخرج كل مفاتيح os.environ.get/os.getenv من ملف Python واحد."""
    try:
...
def _collect_all_env_keys() -> set[str]:
    """يجمع كل مفاتيح البيئة من جميع ملفات Python في APP_DIR."""
...
def _load_documented_keys(env_example: Path) -> set[str]:
    """يستخرج أسماء المتغيرات الموثَّقة من .env.example."""
...
# مفاتيح عامة مُعفاة من التوثيق (Flask internals أو أسماء غامضة)
...
def main() -> int:
    ...
    # نُصفّي المفاتيح العامة والمفاتيح غير الـ UPPER_CASE
    relevant = {
        k for k in all_keys
        if k.isupper() and k not in _EXEMPT
    }
