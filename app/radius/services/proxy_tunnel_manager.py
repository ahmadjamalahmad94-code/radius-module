...
class ProxyTunnelManager:
    ...
    def _write_freeradius_client(
        ...
        *,
        ...
        block = (
            "# AUTO-GENERATED by hoberadius proxy_tunnel_manager — DO NOT EDIT\n"
            # No volatile timestamp here ON PURPOSE — a changing comment would
            # defeat the idempotency guard below and restart FreeRADIUS every
            # heartbeat.
        ...
        # Idempotency guard — only rewrite + trigger a reload when the client
        # config actually changed. The heartbeat calls this every ~5 min; the
        # block used to carry a "Generated at" timestamp AND the trigger was
        # touched unconditionally, so the entrypoint HARD-RESTARTED FreeRADIUS
        # every cycle — a ~1-2s auth outage each time ("RADIUS server is not
        # responding" on the router). With a stable block + this guard, a no-op
        # heartbeat writes nothing and never reloads.
        try:
            if self.clients_path.read_text(encoding="utf-8") == block:
                return True, ()
        ...
        # Config genuinely changed → touch the reload trigger the freeradius
        # entrypoint watcher polls.
