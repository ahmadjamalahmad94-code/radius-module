"""site_exit_presets — VX2.6f built-in seed presets.

Operators frequently start from a canonical list of "things
that need to look like they came from a Saudi-region IP" —
speedtest servers, public-IP checkers, VPN provider pages, etc.
Asking them to paste 300 lines every time is friction.

This module ships those canonical lists as constants. The UI
exposes a dropdown / button row to load any preset into the
seed-import textarea with one click.

Each preset is the same MikroTik address-list format the
importer already understands:

    add address=speedtest.net list=<group>
    add address=104.102.35.193 list=<group>

So loading a preset is the same code path as a manual paste.

Adding a new preset = drop a new `PRESETS[<key>]` entry. No DB
...
class PresetMeta:
    ...
    label_ar: str       # operator-facing Arabic label
    description_ar: str
    target_count: int   # rough line count for the UI hint
    body: str           # the raw seed-file content


# ─── Preset bodies ───────────────────────────────────────────


# karamspeed — the canonical Indonesian/SEA speedtest + ip-checker
# list (shipped as `karamspeed.txt` in many Mikrotik tutorials).
# Source list kept verbatim so the parser sees identical input.
_KARAMSPEED_BODY = """\
...
add address=google.ps list=speedtest
add address=www.google.ps list=speedtest
add address=whatismyip.com list=speedtest
add address=www.whatismyip.com list=speedtest
add address=speedtest.net list=speedtest
add address=webtest.net list=speedtest
add address=speedtestcustom.com list=speedtest
add address=pingtest.net list=speedtest
add address=ooklaserver.net list=speedtest
...
add address=theideroom.net list=speedtest
add address=www.myip.com list=speedtest
...
# A minimal "smoke test" preset — three deterministic targets
# so an operator can verify the pipeline end-to-end in one
# click without committing to 300 entries.
_MINIMAL_SMOKE_BODY = """\
/ip firewall address-list
add address=1.1.1.1 list=smoke
add address=ifconfig.co list=smoke
add address=whatismyip.com list=smoke
"""
...
def list_presets() -> list[PresetMeta]:
    """All registered presets, deterministically ordered."""
    return list(PRESETS.values())


def get_preset(key: str) -> PresetMeta | None:
    """Lookup a single preset by its url-safe key."""
