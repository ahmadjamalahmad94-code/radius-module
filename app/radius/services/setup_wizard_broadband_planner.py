...
class BroadbandBootstrapPlanner:
    def plan(
        ...
        *,
        ...
        lines = [
            "# ════════════════════════════════════════════════",
            "# HobeRadius Broadband / PPPoE Server Setup",
            f"# Run: {wizard_run_id}",
            f"# Tag: {tag}",
            "# ⚠️  WARNING: this script REMOVES any prior",
            "#     HobeRadius PPPoE/broadband config on the",
            "#     same interfaces before adding fresh state.",
            "#     Tag scope: HOBERADIUS_SETUP:<run>:broadband.",
            "#     Existing non-HobeRadius configs are left",
            "#     untouched.",
            "# ════════════════════════════════════════════════",
            "",
            "# ── Cleanup of prior HobeRadius broadband rows ──",
            f'/ip firewall nat remove [find where comment~"{tag}"]',
            f'/interface pppoe-server server remove [find where comment~"{tag}"]',
            f'/ip pool remove [find where comment~"{tag}"]',
            f'/ppp profile remove [find where comment~"{tag}"]',
            "",
            "# --- PPP profile ---",
