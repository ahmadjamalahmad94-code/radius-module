...
def check_card(tenant_id: int, query: str) -> dict:
    ...
    card = {
        ...
        # ── زمنان مختلفان، ولا يُخلطان ──
        #
        # `used_session_seconds` = **مجموع الجلسات الفعليّة**: ما قضاه الزبون
        #   متّصلًا حقًّا. هذا ما يعرضه «الوقت المستخدم» في الواجهة، وهو ما
        #   يقصده المشغّل حين يسأل «كم استخدم».
        #
        # `consumed_seconds` = **زمن النافذة المنقضي** من الميزانية
        #   (‏budget − remaining). في وضع «من أوّل اتّصال» تعدّ النافذة سواءٌ
        #   اتّصل الزبون أم لا، فمن يدخل عشر دقائق ثمّ ينصرف يُظهر بعد خمس
        #   ساعاتٍ «خمس ساعاتٍ منقضية» واستخدامُه عشر دقائق.
        #
        # 🔴 فلا تَعِد بأنّ «المستخدم + المتبقّي = وقت البطاقة»: تصحّ للأوّل
        #    في وضع «بالثانية» فقط، ولا تصحّ للثاني إلّا بمعنى النافذة.
        #    الخلط بينهما هو ما يجعل شاشةً واحدةً تعرض رقمين متناقضين.
        #
