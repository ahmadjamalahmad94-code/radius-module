...
class WizardV3Service:
    ...
    def _render_unified_script(
        ...
        lines = [
            ...
            "# password (pre-generated by the wizard). Restricts",
            "# the /ip service api binding to the VPN tunnel so",
            "# only HobeRadius VPS can connect. Idempotent.",
            f'/user remove [find where name="{api_user}"]',
            f'/user add name="{api_user}" password="{api_password}" group=full comment="{api_tag} {rtag}"',
            "/ip service enable api",
            f'/ip service set api address=10.10.0.1/32',
            "",
