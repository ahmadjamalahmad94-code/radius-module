...
def _covered_by_real_row(tenant_id: int, username: str, mac: str,
    ...
    for r in rows:
        ...
        if rs is None:
            continue
        # 🔴 ثغرةُ الوليدة (مقيسةٌ حيًّا على خادم سمير 2026-08-31): شرطُ
        # التداخل وحدَه يُمسك الجلسةَ القائمةَ ويُفلت التي وُلدت للتوّ.
        # المُصالِح يستطلع بعد ثانيةٍ من بدء الجلسة، فـ`start_dt` يساوي
        # بدايةَ الصفّ الحقيقيّ والتداخلُ **صفر** ⇒ يمرّ ⇒ نسخةٌ ثانية.
        # النتيجة: 110 صفوفٍ مكرَّرة و136.6 ساعةً منفوخةً على 44 حسابًا في
        # يومٍ واحد — بطاقةُ ثلاثِ ساعاتٍ تُعرض 8:27.
        #
        # وصفٌّ حقيقيٌّ **مفتوحٌ** لنفس (المستخدم، الماك) بدايتُه قريبةٌ من
        # البداية المحسوبة **هو هذه الجلسة نفسُها** — لا يحتاج تداخلًا كي
        # يُثبِت ذلك. النافذةُ ±5د تحتمل انحرافَ ساعةِ الراوتر وتقريبَ
        # `uptime`، ولا تبتلع جلسةً أخرى حقيقيّةً بدأت في وقتٍ مختلف.
        still_open = not str(r["acctstoptime"] or "").strip()
        if still_open and abs((rs - start_dt).total_seconds()) <= 300:
            return True
