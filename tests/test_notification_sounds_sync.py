...
def test_pull_stores_central_sounds(app, monkeypatch):
    c = _FakeClient({"router_offline": ("sum-D", b"RIFF" + b"D" * 40)})
    _install(monkeypatch, c)
    with app.app_context():
        rep = sync.sync_once(1)
        assert rep["ok"] and rep["updated"] == 1
        got = snd.resolve(1, event_key="router_offline")
        assert got is not None and got[1] == b"RIFF" + b"D" * 40
        assert snd.status_map(1)["router_offline"]["origin"] == "central"
...
def test_unchanged_checksum_is_not_downloaded_again(app, monkeypatch):
    ...
    with app.app_context():
        sync.sync_once(1)
        assert c.fetches == ["router_offline"]
        rep = sync.sync_once(1)          # سحبةٌ ثانية بلا تغيير
        assert rep["updated"] == 0 and rep["skipped"] == 1
        assert c.fetches == ["router_offline"], "نزّلها مرّتين رغم أنّها لم تتغيّر"


def test_changed_checksum_is_refetched(app, monkeypatch):
    c = _FakeClient({"router_offline": ("sum-1", b"RIFF" + b"1" * 40)})
    _install(monkeypatch, c)
    with app.app_context():
        sync.sync_once(1)
        c.sounds["router_offline"] = ("sum-2", b"RIFF" + b"2" * 40)
        rep = sync.sync_once(1)
        assert rep["updated"] == 1
        assert snd.resolve(1, event_key="router_offline")[1] == b"RIFF" + b"2" * 40
...
def test_unknown_key_from_a_newer_panel_is_skipped_quietly(app, monkeypatch):
    ...
    c = _FakeClient({
        ...
        "router_offline": ("s-d", b"RIFF" + b"D" * 40),
    })
    _install(monkeypatch, c)
    with app.app_context():
        rep = sync.sync_once(1)
        assert rep["ok"] and rep["updated"] == 1 and rep["skipped"] == 1
        assert snd.resolve(1, event_key="router_offline") is not None
...
def test_local_upload_is_never_overwritten_by_central(app, monkeypatch):
    ...
    c = _FakeClient({"router_offline": ("s-c", b"RIFF" + b"C" * 40)})
    _install(monkeypatch, c)
    with app.app_context():
        snd.save_sound(1, "router_offline", b"RIFF" + b"L" * 40,
                       mime="audio/wav", origin="local")
        rep = sync.sync_once(1)
        assert rep["skipped"] == 1 and rep["updated"] == 0
        assert snd.resolve(1, event_key="router_offline")[1] == b"RIFF" + b"L" * 40
