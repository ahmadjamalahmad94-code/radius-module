# -*- coding: utf-8 -*-
"""قالب «الليلي الملكي» (crimson_prestige) — Phase 2 / شركة.

تصميم فاخر مُفرَد في ملفّه الخاصّ، بطلُه رسمة SVG مُضمَّنة لشعار قرمزيّ هندسيّ
...
def test_registered_in_library_and_section():
    ...
    assert "crimson_prestige" in ht.TEMPLATES_BY_SLUG
    t = ht.TEMPLATES_BY_SLUG["crimson_prestige"]
    assert t.name_ar == "القرمزي الفاخر"
    company = [s for s in route._TEMPLATE_SECTIONS if s[0] == "company"][0]
    assert "crimson_prestige" in company[3]


def test_renders_with_embedded_svg_illustration():
    from app.radius.services import hotspot_templates as ht
    html = ht.render("crimson_prestige", {"TENANT_NAME": "نت",
                                          "ACCENT_COLOR": "#DC2626",
                                          "MOTIF_ICON": "wifi"}, tenant_id=1)
    for marker in ("hr-crimson-prestige", "cp-hero", "cp-art", "cp-mark",
                   "<svg", "cp-facets", "عضويّة بريميوم"):
...
def test_login_contracts_intact():
    from app.radius.services import hotspot_templates as ht
    html = ht.render("crimson_prestige", {}, tenant_id=1)
...
def test_watermark_backmost_and_bar_safety_apply():
    from app.radius.services import hotspot_templates as ht
    html = ht.render("crimson_prestige", {"MOTIF_ICON": "wifi"}, tenant_id=1)
...
def test_no_raw_variable_leak():
    from app.radius.services import hotspot_templates as ht
    html = ht.render("crimson_prestige", {"TENANT_NAME": "نت",
                                          "ACCENT_COLOR": "#DC2626"}, tenant_id=1)
...
def test_no_external_image_urls():
    ...
    from app.radius.services import hotspot_template_crimson_prestige as mod
    art = mod._CRIMSON_PRESTIGE_HERO + mod._CRIMSON_PRESTIGE_STYLE
