# -*- coding: utf-8 -*-
"""قالب «الأعمال الرسمي» (corporate_formal) — Phase 2 / شركة.

تصميم فاخر مُفرَد في ملفّه الخاصّ، بطلُه رسمة SVG مُضمَّنة لأفق مدينة أعمال
...
def test_registered_in_library_and_section():
    ...
    assert "corporate_formal" in ht.TEMPLATES_BY_SLUG
    t = ht.TEMPLATES_BY_SLUG["corporate_formal"]
    assert t.name_ar == "الأعمال الرسمي"
    company = [s for s in route._TEMPLATE_SECTIONS if s[0] == "company"][0]
    assert "corporate_formal" in company[3]


def test_renders_with_embedded_svg_illustration():
    from app.radius.services import hotspot_templates as ht
    html = ht.render("corporate_formal", {"TENANT_NAME": "نت",
                                          "ACCENT_COLOR": "#2563EB",
                                          "MOTIF_ICON": "wifi"}, tenant_id=1)
    for marker in ("hr-corporate-formal", "cf-hero", "cf-art", "cf-badge",
                   "<svg", "url(#cfGlass)", "حلول اتصال للأعمال"):
...
def test_login_contracts_intact():
    from app.radius.services import hotspot_templates as ht
    html = ht.render("corporate_formal", {}, tenant_id=1)
...
def test_watermark_backmost_and_bar_safety_apply():
    from app.radius.services import hotspot_templates as ht
    html = ht.render("corporate_formal", {"MOTIF_ICON": "wifi"}, tenant_id=1)
...
def test_no_raw_variable_leak():
    from app.radius.services import hotspot_templates as ht
    html = ht.render("corporate_formal", {"TENANT_NAME": "نت",
                                          "ACCENT_COLOR": "#2563EB"}, tenant_id=1)
...
def test_no_external_image_urls():
    ...
    from app.radius.services import hotspot_template_corporate_formal as mod
    art = mod._CORPORATE_FORMAL_HERO + mod._CORPORATE_FORMAL_STYLE
