...
def test_service_request_creates_support_ticket_without_payment(client):
    ...
    assert data["service_request"]["service_label"] == "خدمة تغيير عنوان الإنترنت العام (Public)"
    assert data["ticket"]["category"] == "service_request"
    assert "الخدمة المطلوبة: خدمة تغيير عنوان الإنترنت العام (Public)" in data["ticket"]["body"]
