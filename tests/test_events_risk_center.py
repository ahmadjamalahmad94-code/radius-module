...
def test_event_filtering_by_category_severity_actor_and_target(app):
    with app.app_context():
        ...
        service = _events_risk_service()(tenant_id=1)
...
def test_entity_timeline_returns_target_events(app):
    with app.app_context():
        ...
        timeline = _events_risk_service()(tenant_id=1).entity_timeline(
...
def test_risk_rules_detect_negative_wallet_and_failed_logins(app):
    with app.app_context():
        wallet = _wallet_service()().create_wallet(tenant_id=1, owner_type="manager", owner_id=1)
        ...
        result = _events_risk_service()(tenant_id=1).run_risk_rules()
...
def test_fraud_flag_creation_records_evidence(app):
    with app.app_context():
        flag = _events_risk_service()(tenant_id=1).create_fraud_flag(
...
def test_revenue_ledger_mismatch_rule_creates_flag(app):
    with app.app_context():
        db().execute(
            ...
            """,
            (1, "manual_sale", 123, 1000, 1000, 0, 1000, 1000, 1000, "JOD", "posted", "{}", _now_iso()),
        )
        result = _events_risk_service()(tenant_id=1).run_risk_rules()
        keys = {flag["flag_key"] for flag in result["flags"]}
