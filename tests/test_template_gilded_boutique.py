# -*- coding: utf-8 -*-
"""قالب «الأعمال الرسمي» (gilded_boutique) — Phase 2 / شركة.

تصميم فاخر مُفرَد في ملفّه الخاصّ، بطلُه رسمة SVG مُضمَّنة لمانيكان فستان بوتيك
...
def test_registered_in_library_and_section():
    ...
    assert "gilded_boutique" in ht.TEMPLATES_BY_SLUG
    t = ht.TEMPLATES_BY_SLUG["gilded_boutique"]
    assert t.name_ar == "البوتيك المذهّب"
    company = [s for s in route._TEMPLATE_SECTIONS if s[0] == "retail"][0]
    assert "gilded_boutique" in company[3]


def test_renders_with_embedded_svg_illustration():
    from app.radius.services import hotspot_templates as ht
    html = ht.render("gilded_boutique", {"TENANT_NAME": "نت",
                                          "ACCENT_COLOR": "#C9A24B",
                                          "MOTIF_ICON": "wifi"}, tenant_id=1)
    for marker in ("hr-gilded-boutique", "gb-hero", "gb-art", "gb-spark",
                   "<svg", "url(#gbGown)", "بوتيك"):
...
def test_login_contracts_intact():
    from app.radius.services import hotspot_templates as ht
    html = ht.render("gilded_boutique", {}, tenant_id=1)
...
def test_watermark_backmost_and_bar_safety_apply():
    from app.radius.services import hotspot_templates as ht
    html = ht.render("gilded_boutique", {"MOTIF_ICON": "wifi"}, tenant_id=1)
...
def test_no_external_image_urls():
    ...
    from app.radius.services import hotspot_template_gilded_boutique as mod
    art = mod._GILDED_BOUTIQUE_HERO + mod._GILDED_BOUTIQUE_STYLE
