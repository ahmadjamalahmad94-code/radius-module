...
def guard(monkeypatch):
    ...
    # النسخة العاديّة تفحص `is_super_admin`، ونسخة الاستضافة تفحص
    # `is_network_owner` (المبدأ غير المقيَّد داخل شبكته). نُحاكي ما هو
    # موجودٌ فعلًا كي يعمل الاختبار على الفرعَين بلا تعارضٍ في الدمج.
    for _name in ("is_super_admin", "is_network_owner"):
        if hasattr(cards_routes, _name):
            monkeypatch.setattr(cards_routes, _name,
                                lambda: state["super"])
