...
def test_events_list_and_filter(app, client):
    ...
    res = client.get("/api/v1/events", headers=AUTH)
    assert res.status_code == 200, res.get_json()
    data = res.get_json()["data"]
    assert data["count"] == 2
    assert "summary" in data
    # فلتر category
    sec = client.get("/api/v1/events?category=security", headers=AUTH).get_json()["data"]
    assert sec["count"] == 1
...
def test_event_detail_and_timeline(app, client):
    ...
    res = client.get(f"/api/v1/events/{eid}", headers=AUTH)
    assert res.status_code == 200, res.get_json()
    data = res.get_json()["data"]
...
def test_risk_list_and_run(app, client):
    res = client.get("/api/v1/events/risk", headers=AUTH)
    assert res.status_code == 200
    assert "flags" in res.get_json()["data"] and "summary" in res.get_json()["data"]
    run = client.post("/api/v1/events/risk/run", headers=AUTH)
...
def test_security_view(app, client):
    ...
    res = client.get("/api/v1/events/security", headers=AUTH)
    assert res.status_code == 200
    data = res.get_json()["data"]
