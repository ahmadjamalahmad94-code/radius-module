# -*- coding: utf-8 -*-
"""قالب «الأعمال الرسمي» (mega_sale) — Phase 2 / شركة.

تصميم فاخر مُفرَد في ملفّه الخاصّ، بطلُه رسمة SVG مُضمَّنة لعربة تسوّق وتخفيضات
...
def test_registered_in_library_and_section():
    ...
    assert "mega_sale" in ht.TEMPLATES_BY_SLUG
    t = ht.TEMPLATES_BY_SLUG["mega_sale"]
    assert t.name_ar == "التخفيضات"
    company = [s for s in route._TEMPLATE_SECTIONS if s[0] == "retail"][0]
    assert "mega_sale" in company[3]


def test_renders_with_embedded_svg_illustration():
    from app.radius.services import hotspot_templates as ht
    html = ht.render("mega_sale", {"TENANT_NAME": "نت",
                                          "ACCENT_COLOR": "#E11D48",
                                          "MOTIF_ICON": "wifi"}, tenant_id=1)
    for marker in ("hr-mega-sale", "ms-hero", "ms-art", "ms-countdown",
                   "<svg", "ms-cart", "تخفيضات"):
...
def test_login_contracts_intact():
    from app.radius.services import hotspot_templates as ht
    html = ht.render("mega_sale", {}, tenant_id=1)
...
def test_watermark_backmost_and_bar_safety_apply():
    from app.radius.services import hotspot_templates as ht
    html = ht.render("mega_sale", {"MOTIF_ICON": "wifi"}, tenant_id=1)
...
def test_no_raw_variable_leak():
    from app.radius.services import hotspot_templates as ht
    html = ht.render("mega_sale", {"TENANT_NAME": "نت",
                                          "ACCENT_COLOR": "#E11D48"}, tenant_id=1)
...
def test_no_external_image_urls():
    ...
    from app.radius.services import hotspot_template_mega_sale as mod
    art = mod._MEGA_SALE_HERO + mod._MEGA_SALE_STYLE
